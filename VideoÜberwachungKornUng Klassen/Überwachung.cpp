...
const string imagePath = "C:\\Users\\Viki\\Dropbox\\KevVic\\AVPRG\\Projekt\\Alarm.jpg";					//Pfad für das Bild, das bei einen Alarm ausgelöst werden soll
Mat image = imread(imagePath);														//Bild laden
VideoÜberwachung videoüberwachungÜ;													//Videoüberwachungsobjekt erstellen

//Methode überprüft, ob sich die übergebenen Point Werte pt1 und pt1 im nicht videoüberwachten Bereich befinden
//und liefert einen Wahrheitswert als Rückgabe (0 für Punkte, die sich ausserhalb des nicht überwachten bereichs befinden,
//1 für Punkte, die sich innerhalb des nicht überwachten Bereichs befinden)
...
//Die Methode calcAverageObjects berechnet den Durchschnitt der gesamten Objekte(Objekte, ungefährliche Objekte und gefährliche Objekte)
//über 25 Frames
...
//Desweiteren zeichnet sie die Anzahl der Objekte in das Videofenster, sowie eine Alarmmeldung (falls Alarm ausgelöst) 
//bei jeden 3 Frame
...
void Überwachung::runÜberwachung(Vector<Point> noCamArray, int videoOrCam){ 
	...
	while(true){
		...
		writeText(gefundeneObjekteSchnitt, ungefaehrlicheObjekte, gefaehrlicheObjekteSchnitt);	//Ausgabe der gezählten Elemente und Alarmnachricht auf Console und in Video
