...
Point mousePoint1, mousePoint2;		//Point für Mauskoordinaten der Rechtecke, die zur Kalibrierung gezeichnet werden

//MouseHandler implementiert eine drag-Funktion und wird im Programm genutzt, um Rechtecke zu zeichnen, während die Maus gedrückt ist.
...
void mCallback(int event, int x, int y, int flags, void* userdata){
	if  ( event == EVENT_LBUTTONDOWN && mouseDragged== false){						
		mousePoint1 = Point(x, y);												//speichert die Mousekoordinate beim Drücken der linken Maustaste
...
//Methode, die das noCamArray mit den Anfangs- und Endkoordinaten für die nicht videoüberwachten Bereiche übergeben bekommt
//und dieses sortiert, so dass die kleineren x- und y-Koordinaten der einzelnen Rechtecke als deren erste Koordinaten gespeichert sind
...
Vector<Point> Kalibrierung::runCalibrate(int NoCamArrayLength		//Anzahl der Bereiche, die nicht vom System überwacht werden sollen
						,Vector<Point> noCamArray					//Point Array mit den Anfangs- und Endpunkten der Rechtecke, welche nicht vom System überwacht werden sollen
						, int videoOrCam){							//Auswahl, ob ein Video abgespielt oder von der Kamera aufgenommen werden soll


	//Initialisierung
	videoüberwachungV.initialize(videoOrCam);									//Initalisierung der Aufnahme bzw. Abspielen des Videos mit Parameter Übergabe für Video oder Kamera


	//Variablendeklaration
	namedWindow("Kalibrierung", 1);												//Kalibrierungsfenster erstellen
	...
	createTrackbar("Kalibrieren", "Kalibrierung",&kalibrierung, 1);				//Kalibrieren Trackbar erstellen mit den Werten 0 und 1 (dient als Button für An/Aus)
	...
	createTrackbar("setGroesse", "Kalibrierung",&setGroesse, 1);				//setGroesse Trackbar erstellen mit den Werten 0 und 1 (dient als Button für An/Aus)
	...
	int rechteckAlarmgroesse = 0;												//Variable für die Rechteckgröße des Rechteckes, mit dem die Alarmgröße für Objekte bestimmt wird 			
	...
	while(true){																//Originalvideo wird abgespielt
		...
		for(int i = 0; i < contours.size(); i++){
			...
			//die Fläche dieses Rechteckes in Pixeln als Alarmgröße gesetzt, sowie die Trackbar auf Position der Alarmgröße gesetzt werden
			...
			//gezeichnet werden, wie beim Start des Programmes angegeben wurde
	...
	//Nach dem Ende der Kalibrierung wird das Array mit den Anfangs- und Endkoordinaten sortiert, so dass 
